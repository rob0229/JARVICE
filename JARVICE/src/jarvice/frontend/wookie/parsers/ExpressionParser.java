...
public class ExpressionParser extends StatementParser {
	...
	static final EnumSet<WookieTokenType> EXPR_START_SET = EnumSet.of(PLUS,
	...
	private static final EnumSet<WookieTokenType> REL_OPS = EnumSet.of(
			EQUALS_EQUALS, NOT_EQUALS, LESS_THAN, LESS_EQUALS, GREATER_THAN,
			GREATER_EQUALS);

	// Map relational operator tokens to node types.
	private static final HashMap<WookieTokenType, ICodeNodeType> REL_OPS_MAP = new HashMap<WookieTokenType, ICodeNodeType>();
	static {
	...
	private ICodeNode parseExpression(Token token) throws Exception {
		...
		if (REL_OPS.contains(tokenType)) {
			...
			ICodeNode simExprNode = parseSimpleExpression(token); // Passing in
																	// 3 here
																	// returns
																	// IcodeNode
			...
			TypeSpec simExprType = simExprNode != null ? simExprNode
	...
	private static final EnumSet<WookieTokenType> ADD_OPS = EnumSet.of(PLUS,
			MINUS, WookieTokenType.OR);

	// Map additive operator tokens to node types.
	private static final HashMap<WookieTokenType, ICodeNodeTypeImpl> ADD_OPS_OPS_MAP = new HashMap<WookieTokenType, ICodeNodeTypeImpl>();
	static {
	...
	private ICodeNode parseSimpleExpression(Token token) throws Exception {
		...
		while (ADD_OPS.contains(tokenType)) {
			...
			ICodeNode termNode = parseTerm(token);// there is a double call to
													// parseTerm here
			opNode.addChild(termNode);// weird I am not sure what is happening
										// here
	...
	private static final EnumSet<WookieTokenType> MULT_OPS = EnumSet.of(STAR,
			SLASH, DIV, WookieTokenType.MOD, WookieTokenType.AND);

	// Map multiplicative operator tokens to node types.
	private static final HashMap<WookieTokenType, ICodeNodeType> MULT_OPS_OPS_MAP = new HashMap<WookieTokenType, ICodeNodeType>();
	static {
